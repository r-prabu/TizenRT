 ...
 ****************************************************************************/
 ...
 /* Per-CPU variable to check the recursive abort */
 static int cpu_abort_state[CONFIG_SMP_NCPUS] = {NORMAL_STATE};
 ...
 void up_assert(const uint8_t *filename, int lineno)
 {
	 ...
	 /* Check if we are in recursive abort on this CPU */
	 if (cpu_abort_state[cpu] == ABORT_STATE) {
		 /* Handle recursive assert - exit critical section and call arm_assert directly */
		 leave_critical_section(flags);
		 arm_assert();
	 } else {
		 /* Mark this CPU as in abort state */
		 cpu_abort_state[cpu] = ABORT_STATE;
	 }
 ...
 #endif
	 {
		 /* Reset the CPU abort state before calling arm_assert() 
		  * This isn't strictly necessary since arm_assert() typically
		  * resets the system, but it's good practice */
		 cpu_abort_state[cpu] = NORMAL_STATE;
		 
		 /* treat kernel fault */
		 arm_assert();
	 }
	 
	 /* Reset the CPU abort state before exiting */
	 cpu_abort_state[cpu] = NORMAL_STATE;
