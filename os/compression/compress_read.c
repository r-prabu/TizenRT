...
static int compress_parse_header(int filfd, uint16_t offset)
{
	...
	if (rpos != offset) {
		...
		bcmpdbg("%s offset %lu errorno %d\n", clog_message_str[CMN_LOG_FAILED_OP], (unsigned long)offset, errval);
		return -errval;
	}
	...
	if (!compression_header) {
		bcmpdbg("%s kmm_malloc for compression_header\n", clog_message_str[CMN_LOG_ALLOC_FAIL]);
		return -ENOMEM;
	}
...
int compress_read(int filfd, uint16_t binary_header_size, FAR uint8_t *buffer, size_t readsize, off_t offset)
{
	...
	for (; index < first_block + no_blocks; index++) {
		...
		if (ret == ERROR) {
			bcmpdbg(" %s decompress %d block of this binary\n", clog_message_str[CMN_LOG_FAILED_OP], index);
			buffer_index = ret;
			goto error_compress_read;
		}
...
int compress_init(int filfd, uint16_t offset, off_t *filelen)
{
	...
	if (ret != OK) {
		bcmpdbg("%s parse compression header from file\n", clog_message_str[CMN_LOG_FAILED_OP]);
